          ...
          searchResults?.map((e, index) => {
            return (
              <div key={index * 2} className='flex flex-col gap-y-3 sm:flex-row gap-x-4 md:gap-x-8 w-[98%] justify-center cursor-pointer sm:mx-0'>
                <img alt="Search Result Thumbnail" onClick={() => pageRoute(`/watch/${e.id?.videoId}`)} className='w-[97%] sm:w-[29%] md:w-[25%] sm:rounded-[23px]' src={e.snippet?.thumbnails?.medium?.url} />
                <div className='w-[92%] sm:w-[60%] md:w-[70%] lg:w-[60%]'>
                  ...
                  <span className='text-[#606060] text-[12px]'>{timeSince(e?.snippet?.publishedAt)}</span>
                  ...
                  <p onClick={() => pageRoute(`/watch/${e.id?.videoId}`)} className='traking-wider font-normal text-[10px] sm:text-[#0f0f0f] text-[13px] leading-[16px]'>{e.snippet?.description ? e.snippet.description.slice(0, 124) + "..." : ""}</p>
                </div>
              </div>
            )
          })
